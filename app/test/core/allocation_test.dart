  ...
  group('allocateByPercent', () {
    test('parts always sum to exactly the total', () {
      ...
      final cases = <(int, Map<String, double>)>[
        ...
        (100000, {'a': 14.28, 'b': 14.28, 'c': 14.29, 'd': 14.29, 'e': 14.28, 'f': 14.29, 'g': 14.29}),
      ...
      for (final (totalMinor, percents) in cases) {
        final result = allocateByPercent(total: Money(totalMinor), percents: percents);
        final summed = result.map((a) => a.amount).sum;
        expect(
          summed.minor,
          totalMinor,
          reason: 'allocation of $totalMinor across $percents lost or invented paise',
        );
      }
    });
    ...
    test('handles an empty map and a zero total', () {
      expect(allocateByPercent(total: const Money(1000), percents: <String, double>{}), isEmpty);
