...
void main() {
  ...
  group('CategoryProgress', () {
    test('reports remaining and status while under budget', () {
      const progress = CategoryProgress(allocated: Money(500000), spent: Money(200000));
    ...
    test('warns before the limit, not at it', () {
      const atSeventyFour = CategoryProgress(allocated: Money(10000), spent: Money(7400));
      const atSeventyFive = CategoryProgress(allocated: Money(10000), spent: Money(7500));
    ...
    test('states overspend as its own quantity, not a negative remainder', () {
      const progress = CategoryProgress(allocated: Money(500000), spent: Money(620000));
    ...
    test('clamps the progress bar but not the underlying ratio', () {
      const progress = CategoryProgress(allocated: Money(1000), spent: Money(1400));
      expect(progress.ratio, closeTo(1.4, 0.001));
      expect(progress.displayRatio, 1.0);
    });

    test('an unallocated category is not 100% spent', () {
      const progress = CategoryProgress(allocated: Money.zero(), spent: Money.zero());
  ...
  group('suggestReallocation', () {
    final categories = {
      'food': const CategoryProgress(allocated: Money(500000), spent: Money(620000)),
      'transport': const CategoryProgress(allocated: Money(300000), spent: Money(100000)),
      'shopping': const CategoryProgress(allocated: Money(200000), spent: Money(150000)),
    };
    ...
    test('says nothing rather than inventing a source', () {
      final exhausted = {
        'food': const CategoryProgress(allocated: Money(1000), spent: Money(2000)),
        'bills': const CategoryProgress(allocated: Money(1000), spent: Money(1000)),
      };
      expect(
        suggestReallocation(
